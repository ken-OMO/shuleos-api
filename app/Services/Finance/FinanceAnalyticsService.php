...
class FinanceAnalyticsService
{
    ...
    public function summary(User $user): array
    {
        ...
        $collected = $payments->sum(fn ($row) => $this->money->minor($row->allocated_amount));
        ...
        $allocationMismatches = $invoices->filter(function ($invoice) use ($user) {
            $allocated = DB::table('payment_allocations')->where('school_id', $user->school_id)->where('invoice_id', $invoice->id)->where('status', 'active')->get()->sum(fn ($allocation) => $this->money->minor($allocation->allocated_amount));
