...
class SchemeLessonController extends BaseCrudController
{
    ...
    public function index(Request $request)
    {
        ...
        foreach (['scheme_id', 'week_id'] as $f) {
    ...
    public function store(Request $request)
    {
        ...
        $lesson = DB::transaction(function () use ($request, $v) {
            $lesson = $this->service->create($v, $this->schoolId($request, $v));
            $this->audit($request, self::MODULE, 'Create', $lesson, null, $lesson->toArray(), 'Created scheme lesson.');
    ...
    {
        $lesson = $this->tenantQuery($request)->find($id);
        if (! $lesson) {
            return $this->notFound('Scheme lesson not found.');
        }
        ...
        if (isset($v['lesson_number']) && SchemeLesson::where('scheme_id', $lesson->scheme_id)->where('lesson_number', $v['lesson_number'])->whereKeyNot($id)->exists()) {
            return $this->validation(['lesson_number' => ['This lesson number already exists in the scheme.']]);
        }
        if (isset($v['week_id']) && ! AcademicWeek::whereKey($v['week_id'])->where('school_id', $lesson->scheme->school_id)->where('academic_year_id', $lesson->scheme->academic_year_id)->where('term_id', $lesson->scheme->term_id)->where('active', true)->exists()) {
            return $this->validation(['week_id' => ['The week does not belong to the scheme academic period.']]);
        }
        $old = $lesson->toArray();
        DB::transaction(function () use ($request, $lesson, $v, $old) {
            $lesson->update($v);
            $this->audit($request, self::MODULE, 'Update', $lesson, $old, $lesson->fresh()->toArray(), 'Updated scheme lesson.');
        });
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        DB::transaction(function () use ($request, $lesson) {
            $old = $lesson->toArray();
            $lesson->update(['is_deleted' => true, 'deleted_at' => now(), 'deleted_by' => auth()->id()]);
            $this->audit($request, self::MODULE, 'Delete', $lesson, $old, $lesson->toArray(), 'Soft deleted scheme lesson.');
        });

        return $this->success(null, 'Scheme lesson deleted successfully.');
    }

    private function tenantQuery(Request $r)
    {
        return SchemeLesson::current()->whereHas('scheme', fn ($q) => $q->where('school_id', $this->schoolId($r))->where('is_deleted', false));
    }

    private function schoolId(Request $r, array $v = []): string
    {
