...
return new class extends Migration
{
    ...
    {
        Schema::create('teacher_portal_devices', function (Blueprint $table) {
        ...
        Schema::create('teacher_contact_change_requests', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('school_id')->constrained('schools');
            $table->foreignUuid('user_id')->constrained('users');
        ...
        Schema::create('teacher_portal_audit_logs', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('school_id')->constrained('schools');
            $table->foreignUuid('user_id')->constrained('users');
        ...
        Schema::table('teacher_dashboard_preferences', function (Blueprint $table) {
            $table->string('preferred_language', 10)->default('en');
            $table->string('timezone', 80)->default('Africa/Nairobi');
            $table->uuid('default_assignment_id')->nullable();
            $table->uuid('default_stream_id')->nullable();
            $table->string('timetable_view', 20)->default('week');
        });
    }
